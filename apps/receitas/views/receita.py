...
def index(request):
    '''Carrega a página inicial com todas as receitas publicadas.'''
...
def receita(request, receita_id):
    '''Exibe a receita que o usuário quer ver.'''
...
def cria_receita(request):
    '''Aba para criar receita, carregando os seus devidos campos.'''
...
def edita_receita(request, receita_id):
    '''Exibe um botão para edição de suas receitas na área de dashboard.'''
    receita = get_object_or_404(Receita, pk=receita_id)
    receita_a_editar = { 'receita': receita}
    return render(request, 'receitas/edita_receita.html', receita_a_editar)

def atualiza_receita(request):
    '''Confirma a atualização de receitas.'''
    if request.method == 'POST':
